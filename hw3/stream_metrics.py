...
def print_counts(rdd):
    ...
    print("15_second_count={15}; 60_second_count={60}; total_count={total};".format(**counts))
...
if __name__ == '__main__':
    ...
    kvs = KafkaUtils.createStream(ssc, agrs.zookeeper, "consumer-HW3StreamMetricsComputer", {args.topic: 4})
    ...
    15_second_count = unsuccessful_queries.count()
    60_second_count = unsuccessful_queries.countByWindow(60, 15)
    total_count = 15_second_count.map(lambda x: ('total', x)).updateStateByKey(update_function)
    counts = ssc.union([15_second_count.map(lambda x: ('15', x)),
                        60_second_count.map(lambda x: ('60', x)),
                        total_count])
